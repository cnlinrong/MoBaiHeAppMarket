...
//        layoutParams.width = (gridItem.getWidth() > 0 ? gridItem.getWidth() : gridItem.getColumnSize() * mBaseWidth)
//                + ((gridItem.getColumnSize() > 1 && gridItem.getWidth() <= 0 ? mHorizontalMargin * (gridItem.getColumnSize() - 1) : 0));
//        layoutParams.height = (gridItem.getHeight() > 0 ? gridItem.getHeight() : gridItem.getRowSize() * mBaseHeight)
//                + ((gridItem.getRowSize() > 1 && gridItem.getWidth() <= 0 ? mVerticalMargin * (gridItem.getRowSize() - 1) : 0));

        layoutParams.width = gridItem.getWidth();
        layoutParams.height = gridItem.getHeight();
...
//        layoutParams.setGravity(gridItem.getGravity());
        ...
        layoutParams.topMargin = mHorizontalMargin;
        layoutParams.leftMargin = mHorizontalMargin;
        layoutParams.rightMargin = mHorizontalMargin;
        layoutParams.bottomMargin = mHorizontalMargin;
    ...
    private void measure() {
        ...
        for (GridItem item : mGridItemList) {
            ...
            if (rowMap.containsKey(row)) {
                rowMap.put(row, rowMap.get(row) + item.getColumnSize());
            } else {
                rowMap.put(row, item.getColumnSize());
            }

            if (columnMap.containsKey(column)) {
                columnMap.put(column, columnMap.get(column) + item.getRowSize());
            } else {
                columnMap.put(column, item.getRowSize());
            }
        }
...
//        }
	...
	public GridBuilder setColumnCount(int mColumnCount) {
	...
	public GridBuilder setOrientation(int orientation) {
		this.orientation = orientation;
		
		this.mGridLayout.setOrientation(orientation);
