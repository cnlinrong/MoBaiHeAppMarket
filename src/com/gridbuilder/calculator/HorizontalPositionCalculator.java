...
//                Log.e("MY_LOG", item.tag + " : " + item.getColumnSize() + "," + item.getRowSize() + "," + i + "," + j);

                for (int k = i; k < i + item.getColumnSize(); k++) {
                    for (int l = j; l < j + item.getRowSize(); l++) {
                        mMatrix[k][l] = 1;
                    }
                }
    ...
    private boolean canPutDown(GridItem item, int x, int y) {
        for (int k = x; k < x + item.getColumnSize(); k++) {
            for (int l = y; l < y + item.getRowSize(); l++) {
                if (mMatrix[k][l] == 1) {
                    return false;
                }
            }
